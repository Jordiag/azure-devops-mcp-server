...
public class OverviewTools(IWikiClient wikiClient, ISummaryClient summaryClient, IDashboardClient dashboardClient, ILogger<OverviewTools> logger)
{
    ...
    public async Task<Guid> CreateWikiAsync(WikiCreateOptions options) =>
        (await _wikiClient.CreateWikiAsync(options)).EnsureSuccess(_logger);
    ...
    public async Task<WikiV2> GetWikiAsync(Guid wikiId) =>
        (await _wikiClient.GetWikiAsync(wikiId)).EnsureSuccess(_logger);
    ...
    public async Task<IReadOnlyList<WikiV2>> ListWikisAsync() =>
        (await _wikiClient.ListWikisAsync()).EnsureSuccess(_logger);
    ...
    public async Task<WikiV2> DeleteWikiAsync(Guid wikiId) =>
        (await _wikiClient.DeleteWikiAsync(wikiId)).EnsureSuccess(_logger);
    ...
    public async Task<int> CreateOrUpdatePageAsync(Guid wikiId, WikiPageUpdateOptions options, GitVersionDescriptor gitVersionDescriptor) =>
        (await _wikiClient.CreateOrUpdatePageAsync(wikiId, options, gitVersionDescriptor)).EnsureSuccess(_logger);
    ...
    public async Task<Dashboard> GetDashboardAsync(Guid dashboardId, string teamName) =>
        (await _dashboardClient.GetDashboardAsync(dashboardId, teamName)).EnsureSuccess(_logger);
    ...
    public async Task<IReadOnlyList<Dashboard>> ListDashboardsAsync() 
        => (await _dashboardClient.ListDashboardsAsync()).EnsureSuccess(_logger);
}

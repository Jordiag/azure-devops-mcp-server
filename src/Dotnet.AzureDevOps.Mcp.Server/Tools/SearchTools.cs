...
public class SearchTools(ISearchClient searchClient, ILogger<SearchTools> logger)
{
    ...
    public async Task<string> SearchCodeAsync(CodeSearchOptions options) =>
        (await _searchClient.SearchCodeAsync(options)).EnsureSuccess(_logger);
    ...
    public async Task<string> SearchWikiAsync(WikiSearchOptions options) =>
        (await _searchClient.SearchWikiAsync(options)).EnsureSuccess(_logger);
    ...
    public async Task<string> SearchWorkItemsAsync(WorkItemSearchOptions options) =>
        (await _searchClient.SearchWorkItemsAsync(options)).EnsureSuccess(_logger);
    ...
    public async Task<bool> IsCodeSearchEnabledAsync() =>
        (await _searchClient.IsCodeSearchEnabledAsync()).EnsureSuccess(_logger);
}
